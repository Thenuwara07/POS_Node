...
import { QuickSaleItemDto } from './quick-sale-item.dto';
...
export class CreateQuickSaleDto {
  @ApiProperty({
    ...
    description: 'At least one quick sale item is required.',
  })
  @Transform(({ value }) =>
  ...
  @IsString()
  @IsIn(['Cash', 'Card'])
  payment_type: 'Cash' | 'Card' = 'Cash';

  @ApiPropertyOptional({
    name: 'sale_invoice_id',
    example: 'INV-QUICK-001',
  ...
  @ApiPropertyOptional({
    name: 'remain_amount',
    example: 0,
    description: 'Outstanding amount, defaults to 0',
  })
  @Transform(({ value }) => (value == null ? 0 : Number(value)))
  @IsNumber()
  @Min(0)
