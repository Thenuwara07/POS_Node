...
export class ImageStorageService {
  /**
   * Saves a Base64 data URL like "data:image/png;base64,...." for ITEM images.
  ...
  saveBase64ItemImage(dataUrl: string): string {
    ...
    const mime = match[1];
    const ext = mime.endsWith('png') ? '.png' : mime.includes('webp') ? '.webp' : '.jpg';
    const b64 = match[3];
  ...
  /**
   * Saves a Base64 data URL for CATEGORY images.
   * Returns a relative path like "uploads/categories/2025/10/<file>.png"
   */
  saveBase64CategoryImage(dataUrl: string): string {
    const match = dataUrl.match(/^data:(image\/(png|jpeg|jpg|webp));base64,(.+)$/i);
    if (!match) throw new BadRequestException('Invalid base64 image data URL');
    ...
    const now = new Date();
    const relDir = join('uploads', 'categories', `${now.getFullYear()}`, `${(now.getMonth() + 1).toString().padStart(2, '0')}`);
    const absDir = join(process.cwd(), relDir);
    if (!existsSync(absDir)) mkdirSync(absDir, { recursive: true });
